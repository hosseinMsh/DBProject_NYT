...
def process_upload(request: HttpRequest, pk: int):
    ...
    uf.status = "processing"; uf.save(update_fields=["status"])
...
def process_urls(request: HttpRequest):
    ...
    for item in pending:
        item.status = "processing"; item.save(update_fields=["status"])
        try:
            ...
            if item.kind == "parquet":
                ...
                rows = _ingest_parquet(path)
            item.processed_rows = rows; item.status = "done"
            item.save(update_fields=["processed_rows","status"])
        except Exception as e:
            item.status = "error"; item.error_message = str(e)
